...
ofstream treeFile("tree.txt", ios::out | ios::binary);  // открываем файл для записи 
...
public:
	...
	void tabCreater()
	{
		if (left != nullptr)
		{
			hufCode.push_back(0); // добавляем в конец вектора 0
			left->tabCreater();
		}
		if (right != nullptr)
		{
			hufCode.push_back(1); // добавляем в конец вектора 1
			right->tabCreater(); // рекурсия
		}
		if (left == nullptr && right == nullptr) trueTab[ch] = hufCode; // заносим код символа в мапу trueTab
		if (!hufCode.empty()) hufCode.pop_back(); //удаляем последнюю ячейку
	}

	bool zip(string fileName)
	{
		ifstream input(fileName, ios::out | ios::binary); // открываем файл на потоковое двоичное чтение
		if (!input.is_open()) return 0; // выходи если не открылся
